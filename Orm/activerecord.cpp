...
bool ActiveRecord::load(const QString &column_pk, const QString &column, const QVariant &value)
{
    bool result = false;

    if (m_isValid) {
        QSqlQuery qr = Query::select(m_entity, m_connectionName)
                .where(QString("%1 = ?").arg(column), QVariantList() << value).make();
        if (qr.exec()) {
            if (qr.next()) {
                m_record.clear();
                ...
                m_primaryKey.setValue(m_record.value(column_pk));
                result = m_isPersistent = true;
            } else {
                m_lastError = "object does not persists";
            }
